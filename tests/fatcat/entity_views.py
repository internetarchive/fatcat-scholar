...
def test_release_bibtex(client, fcclient, entities):
    ...
    rv = client.get(f"/fatcat/release/{r.ident}.bib")
...
def test_release_citeproc(client, fcclient, entities):
    ...
    rv = client.get(f"/fatcat/release/{r.ident}/citeproc")
    ...
    rv = client.get(f"/fatcat/release/{r.ident}/citeproc?style=modern-language-association")
    ...
    rv = client.get(f"/fatcat/release/{r.ident}/citeproc?style=csl-json")
    ...
    rv = client.get(f"/fatcat/release/{r.ident}/citeproc?style=elsevier-harvard")
    ...
    rv = client.get(f"/fatcat/release/{r.ident}/citeproc?style=bad")
...
def test_entity_history_views(client, fcclient, entities, histories):
    ...
    rv = client.get(f"/fatcat/container/{c.ident}/history")
    assert rv.status_code == 200
    ...
    cr = entities["creator"]
    rv = client.get(f"/fatcat/creator/{cr.ident}/history")
    assert rv.status_code == 200
    ...
    f = entities["file"]
    rv = client.get(f"/fatcat/file/{f.ident}/history")
    assert rv.status_code == 200
    ...
    fs = entities["fileset"]
    rv = client.get(f"/fatcat/fileset/{fs.ident}/history")
    assert rv.status_code == 200
    ...
    rv = client.get(f"/fatcat/webcapture/{wc.ident}/history")
    assert rv.status_code == 200
    ...
    r = entities["release"]
    rv = client.get(f"/fatcat/release/{r.ident}/history")
    assert rv.status_code == 200
    ...
    w = entities["work"]
    rv = client.get(f"/fatcat/work/{w.ident}/history")
    assert rv.status_code == 200
    assert "#4529728" in rv.text
    assert "crossref-bot" in rv.text
...
def test_malformed_entity(client, entity_types):
    for entity_type in entity_types:
        rv = client.get("/fatcat/{}/9999999999".format(entity_type))
        ...
        #rv = client.get("/fatcat/{}/ccccccccccccccccccccccccca".format(entity_type))
        #assert rv.status_code == 404, "mystery {entity_type}"
        rv = client.get("/fatcat/{}/9999999999/history".format(entity_type))
        assert rv.status_code == 422, f"malformed {entity_type} history"
        rv = client.get("/fatcat/{}/f1f046a3-45c9-ffff-ffff-ffffffffffff".format(entity_type))
        assert rv.status_code == 422, f"malformed {entity_type} uuid"
        rv = client.get("/fatcat/{}/rev/f1f046a3-45c9-ffff-ffff-fffffffff".format(entity_type))
        assert rv.status_code == 422, "malformed {entity_type} rev"

def test_search_redirects(client):
    cases = [{"name": "blank redirect",
              "url": "/fatcat/search",
              "redirect_path": "/fatcat/release/search",
              "qarg": [],
              },
             {"name": "generic multi-term",
              "url": "/fatcat/search?q=foo%20bar",
              "redirect_path": "/fatcat/release/search",
              "qarg": ["q=foo%2Bbar", "generic=1"],},
             {"name": "doi",
              "url": "/fatcat/search?q=10.3390/arts8010040",
              "redirect_path": "/fatcat/release/lookup",
              "qarg": ["doi=10.3390%2Farts8010040"],},
             {"name":"pmcid",
              "url": "/fatcat/search?q=PMC5160550",
              "redirect_path": "/fatcat/release/lookup",
              "qarg": ["pmcid=PMC5160550"],},
             {"name": "issn",
              "url": "/fatcat/search?q=2333-2468",
              "redirect_path": "/fatcat/container/lookup",
              "qarg": ["issnl=2333-2468"],},
             {"name": "isbn13",
              "url": "/fatcat/search?q=978-0-12-415894-8",
              "redirect_path": "/fatcat/release/lookup",
              "qarg": ["isbn13=978-0-12-415894-8"], },
             {"name": "arxiv_id",
              "url": "/fatcat/search?q=0712.2293v1",
              "redirect_path": "/fatcat/release/lookup",
              "qarg": ["arxiv=0712.2293v1"], },
             {"name": "sha1",
              "url": "/fatcat/search?q=5bcf14bb375d294162c671b214da7771064d7ba7",
              "redirect_path": "/fatcat/file/lookup",
              "qarg": ["sha1=5bcf14bb375d294162c671b214da7771064d7ba7"], },
             {"name": "sha256",
              "url": "/fatcat/search?q=9f089668c7f604097ad250983ba27201b63272ea4b954c5f3e4e94d668e60cb4",
              "redirect_path": "/fatcat/file/lookup",
              "qarg": ["sha256=9f089668c7f604097ad250983ba27201b63272ea4b954c5f3e4e94d668e60cb4"], },
             {"name": "orcid",
              "url": "/fatcat/search?q=0000-0003-3118-6859",
              "redirect_path": "/fatcat/creator/lookup",
              "qarg": ["orcid=0000-0003-3118-6859"], },
             {"name": "generic single term",
              "url": "/fatcat/search?q=foobar",
              "redirect_path": "/fatcat/release/search",
...
def test_creator_lookup(client, fcclient, entities):
    ...
    for extidtype, extid in (("orcid", c.orcid), ("wikidata_qid", c.wikidata_qid)):
        rv = client.get(f"/fatcat/creator/lookup?{extidtype}={extid}", follow_redirects=False)
...
def test_release_lookup(client, fcclient, entities):
    ...
    for extidtype, extid in extidtypes:
        rv = client.get(f"/fatcat/release/lookup?{extidtype}={extid}", follow_redirects=False)
...
def test_file_lookup(client, fcclient, entities):
    ...
    for extidtype, extid in extidtypes:
        rv = client.get(f"/fatcat/file/lookup?{extidtype}={extid}", follow_redirects=False)
...
def test_container_lookup(client, fcclient, entities):
    ...
    for extidtype, extid in extidtypes:
        rv = client.get(f"/fatcat/container/lookup?{extidtype}={extid}", follow_redirects=False)
...
def test_container_ident_browse(client, fcclient, es, es_resps, entities):
    ...
    rv = client.get(f"/fatcat/container/{c.ident}/browse", follow_redirects=False)
    assert rv.status_code == 302

    c.state = "deleted"
    fcclient.get_container.return_value = c
    rv = client.get(f"/fatcat/container/{c.ident}/browse")
    assert rv.status_code == 200
    ...
    rv = client.get(f"/fatcat/container/{c.ident}/browse?year=1969&issue=6&volume=13")
    assert rv.status_code == 200
    assert "Mourning" in rv.text
    assert "Volume 13, Issue 6 (1969)" in rv.text

def test_container_search(client, es, es_resps):
    rv = client.get("/fatcat/container/search")
    assert rv.status_code == 200
    ...
    rv = client.get("/fatcat/container/search?q=tetsuo")
    assert rv.status_code == 200
...
def test_container_ident_search(client, fcclient, es, es_resps, entities):
    ...
    rv = client.get(f"/fatcat/container/{c.ident}/search")
    assert rv.status_code == 200
    ...
    rv = client.get(f"/fatcat/container/{c.ident}/search?q=gecko")
    assert rv.status_code == 200
    assert "Mourning" in rv.text
    assert "out of 2 results" in rv.text
...
def test_generic_entity_view_active_release(client, fcclient, mocker, entities):
    ...
    res = client.get("/fatcat/release/abcdefghijklmnopqrstuvwxyz")
...
def test_generic_entity_view_redirect_release(client, fcclient, mocker, entities):
    ...
    r.state = "redirect"
    fcclient.get_release = mocker.MagicMock(return_value=r)
    res = client.get("/fatcat/release/abcdefghijklmnopqrstuvwxyz", follow_redirects=False)
    assert res.status_code == 302

def test_generic_entity_view_webcapture_metadata(client, fcclient, entities):
    wc = entities["webcapture"]
    fcclient.get_webcapture.return_value = wc
    res = client.get("/fatcat/webcapture/abcdefghijklmnopqrstuvwxyz/metadata")
    assert res.status_code == 200
...
def test_generic_entity_view_release_metadata(client, fcclient, entities):
    r = entities["release"]
    fcclient.get_release.return_value = r
    res = client.get("/fatcat/release/abcdefghijklmnopqrstuvwxyz/metadata", follow_redirects=False)
    assert res.status_code == 200
...
def test_generic_entity_view_release_contribs(client, fcclient, entities):
    r = entities["release"]
    fcclient.get_release.return_value = r
    res = client.get("/fatcat/release/abcdefghijklmnopqrstuvwxyz/contribs")
    assert res.status_code == 200
...
def test_generic_entity_view_container_view(client, fcclient, es, entities, es_resps):
    ...
    res = client.get("/fatcat/container/abcdefghijklmnopqrstuvwxyz")
    assert res.status_code == 200
...
def test_generic_entity_view_container_view_coverage(client, fcclient, mocker, entities):
    ...
    fcclient.get_container.return_value = c
    m1 = mocker.patch("scholar.fatcat.web.get_elastic_container_stats", return_value={"total":0})
    m2 = mocker.patch("scholar.fatcat.web.get_elastic_preservation_by_type", return_value=[{}])

    res = client.get("/fatcat/container/abcdefghijklmnopqrstuvwxyz/coverage")
...
def test_generic_entity_view_creator(client, fcclient, entities):
    ...
    res = client.get("/fatcat/creator/abcdefghijklmnopqrstuvwxyz")
...
def test_generic_entity_view_file(client, fcclient, entities):
    ...
    res = client.get("/fatcat/file/abcdefghijklmnopqrstuvwxyz")
    assert res.status_code == 200
...
def test_generic_entity_view_fileset(client, fcclient, entities):
    ...
    res = client.get("/fatcat/fileset/abcdefghijklmnopqrstuvwxyz")
...
def test_generic_entity_view_webcapture(client, fcclient, entities):
    wc = entities["webcapture"]
    fcclient.get_webcapture.return_value = wc
    res = client.get("/fatcat/webcapture/abcdefghijklmnopqrstuvwxyz")
    assert res.status_code == 200
    assert wc.cdx[0].sha1 in res.text
...
def test_generic_entity_view_work(client, fcclient, entities):
    ...
    res = client.get("/fatcat/work/abcdefghijklmnopqrstuvwxyz")
    assert res.status_code == 200
...
def test_generic_entity_view_work_metadata(client, fcclient, entities):
    w = entities["work"]
    fcclient.get_work.return_value = w
    res = client.get("/fatcat/work/abcdefghijklmnopqrstuvwxyz/metadata")
    assert res.status_code == 200
...
def test_generic_entity_view_fileset_metadata(client, fcclient, entities):
    fs = entities["fileset"]
    fcclient.get_fileset.return_value = fs
    res = client.get("/fatcat/fileset/abcdefghijklmnopqrstuvwxyz/metadata")
    assert res.status_code == 200
    assert fs.manifest[0].path in res.text
...
def test_generic_entity_view_file_metadata(client, fcclient, entities):
    f = entities["file"]
    fcclient.get_file.return_value = f
    res = client.get("/fatcat/file/abcdefghijklmnopqrstuvwxyz/metadata")
    assert res.status_code == 200
    assert f.md5 in res.text
    assert f.sha256 in res.text
...
def test_generic_entity_view_creator_metadata(client, fcclient, entities):
    ...
    res = client.get("/fatcat/creator/abcdefghijklmnopqrstuvwxyz/metadata")
...
def test_generic_entity_view_container_view_metadata(client, fcclient, mocker, entities):
    c = entities["container"]
    fcclient.get_container.return_value = c
    res = client.get("/fatcat/container/abcdefghijklmnopqrstuvwxyz/metadata")
    assert res.status_code == 200
    assert "urusei yatsura" in res.text

def test_generic_entity_views(client, mocker):
    cases = [{"route": "/fatcat/container/abcdefghijklmnopqrstuvwxyz/coverage",
               "args": ["container", "abcdefghijklmnopqrstuvwxyz", "container_view_coverage.html"]},
             {"route": "/fatcat/container_abcdefghijklmnopqrstuvwxyz",
               "args": ["container", "abcdefghijklmnopqrstuvwxyz", "container_view.html"]},
             {"route": "/fatcat/container/abcdefghijklmnopqrstuvwxyz",
               "args": ["container", "abcdefghijklmnopqrstuvwxyz", "container_view.html"]},
             {"route": "/fatcat/container/abcdefghijklmnopqrstuvwxyz/metadata",
               "args": ["container", "abcdefghijklmnopqrstuvwxyz", "entity_view_metadata.html"]},

             {"route": "/fatcat/file_abcdefghijklmnopqrstuvwxyz",
               "args": ["file", "abcdefghijklmnopqrstuvwxyz", "file_view.html"]},
             {"route": "/fatcat/file/abcdefghijklmnopqrstuvwxyz",
               "args": ["file", "abcdefghijklmnopqrstuvwxyz", "file_view.html"]},
             {"route": "/fatcat/file/abcdefghijklmnopqrstuvwxyz/metadata",
               "args": ["file", "abcdefghijklmnopqrstuvwxyz", "entity_view_metadata.html"]},

             {"route": "/fatcat/fileset_abcdefghijklmnopqrstuvwxyz",
               "args": ["fileset", "abcdefghijklmnopqrstuvwxyz", "fileset_view.html"]},
             {"route": "/fatcat/fileset/abcdefghijklmnopqrstuvwxyz",
               "args": ["fileset", "abcdefghijklmnopqrstuvwxyz", "fileset_view.html"]},
             {"route": "/fatcat/fileset/abcdefghijklmnopqrstuvwxyz/metadata",
               "args": ["fileset", "abcdefghijklmnopqrstuvwxyz", "entity_view_metadata.html"]},

             {"route": "/fatcat/creator/abcdefghijklmnopqrstuvwxyz",
               "args": ["creator", "abcdefghijklmnopqrstuvwxyz", "creator_view.html"]},
             {"route": "/fatcat/creator_abcdefghijklmnopqrstuvwxyz",
               "args": ["creator", "abcdefghijklmnopqrstuvwxyz", "creator_view.html"]},
             {"route": "/fatcat/creator/abcdefghijklmnopqrstuvwxyz/metadata",
               "args": ["creator", "abcdefghijklmnopqrstuvwxyz", "entity_view_metadata.html"]},

             {"route": "/fatcat/release_abcdefghijklmnopqrstuvwxyz",
               "args": ["release", "abcdefghijklmnopqrstuvwxyz", "release_view.html"]},
             {"route": "/fatcat/release/abcdefghijklmnopqrstuvwxyz",
               "args": ["release", "abcdefghijklmnopqrstuvwxyz", "release_view.html"]},
             {"route": "/fatcat/release/abcdefghijklmnopqrstuvwxyz/contribs",
               "args": ["release", "abcdefghijklmnopqrstuvwxyz", "release_view_contribs.html"]},
             {"route": "/fatcat/release/abcdefghijklmnopqrstuvwxyz/references",
               "args": ["release", "abcdefghijklmnopqrstuvwxyz", "release_view_references.html"]},
             {"route": "/fatcat/release/abcdefghijklmnopqrstuvwxyz/metadata",
               "args": ["release", "abcdefghijklmnopqrstuvwxyz", "entity_view_metadata.html"]},

             {"route": "/fatcat/webcapture/abcdefghijklmnopqrstuvwxyz",
               "args": ["webcapture", "abcdefghijklmnopqrstuvwxyz", "webcapture_view.html"]},
             {"route": "/fatcat/webcapture_abcdefghijklmnopqrstuvwxyz",
               "args": ["webcapture", "abcdefghijklmnopqrstuvwxyz", "webcapture_view.html"]},
             {"route": "/fatcat/webcapture/abcdefghijklmnopqrstuvwxyz/metadata",
               "args": ["webcapture", "abcdefghijklmnopqrstuvwxyz", "entity_view_metadata.html"]},

             {"route": "/fatcat/work/abcdefghijklmnopqrstuvwxyz/metadata",
               "args": ["work", "abcdefghijklmnopqrstuvwxyz", "entity_view_metadata.html"]},
             {"route": "/fatcat/work_abcdefghijklmnopqrstuvwxyz",
               "args": ["work", "abcdefghijklmnopqrstuvwxyz", "work_view.html"]},
             {"route": "/fatcat/work/abcdefghijklmnopqrstuvwxyz",
               "args": ["work", "abcdefghijklmnopqrstuvwxyz", "work_view.html"]},
            ]

    for case in cases:
        mocker.patch("scholar.fatcat.web.generic_entity_view")
        client.get(case["route"])
        scholar.fatcat.web.generic_entity_view.assert_called_once()
        calls = scholar.fatcat.web.generic_entity_view.call_args[0]
...
def test_generic_entity_revision_routes(client, mocker):
    cases = [{"route": "/fatcat/container/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d",
               "args": ["container", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "container_view.html"]},
             {"route": "/fatcat/container/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata",
               "args": ["container", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "entity_view_metadata.html"]},

             {"route": "/fatcat/creator/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d",
               "args": ["creator", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "creator_view.html"]},
             {"route": "/fatcat/creator/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata",
               "args": ["creator", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "entity_view_metadata.html"]},

             {"route": "/fatcat/file/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d",
               "args": ["file", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "file_view.html"]},
             {"route": "/fatcat/file/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata",
               "args": ["file", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "entity_view_metadata.html"]},

             {"route": "/fatcat/webcapture/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d",
               "args": ["webcapture", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "webcapture_view.html"]},
             {"route": "/fatcat/webcapture/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata",
               "args": ["webcapture", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "entity_view_metadata.html"]},

             {"route": "/fatcat/fileset/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d",
               "args": ["fileset", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "fileset_view.html"]},
             {"route": "/fatcat/fileset/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata",
               "args": ["fileset", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "entity_view_metadata.html"]},

             {"route": "/fatcat/work/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d",
               "args": ["work", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "work_view.html"]},
             {"route": "/fatcat/work/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata",
               "args": ["work", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "entity_view_metadata.html"]},

             {"route": "/fatcat/release/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d",
               "args": ["release", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "release_view.html"]},
             {"route": "/fatcat/release/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata",
               "args": ["release", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "entity_view_metadata.html"]},
             {"route": "/fatcat/release/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/contribs",
               "args": ["release", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "release_view_contribs.html"]},
             {"route": "/fatcat/release/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/references",
               "args": ["release", UUID("a078e5fe-0815-4ec4-82d8-7841b8a6317d"), "release_view_references.html"]},
            ]

    for case in cases:
        mocker.patch("scholar.fatcat.web.generic_entity_revision_view")
        client.get(case["route"])
        scholar.fatcat.web.generic_entity_revision_view.assert_called_once()
        calls = scholar.fatcat.web.generic_entity_revision_view.call_args[0]
...
def test_generic_entity_revision_view_release(client, fcclient, mocker, entities):
    ...
    res = client.get("/fatcat/release/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d")
    assert res.status_code == 200
    assert r.title in res.text
    assert r.publisher in res.text
...
def test_generic_entity_revision_view_release_metadata(client, fcclient, entities):
    r = entities["release"]
    fcclient.get_release_revision.return_value = r
    res = client.get("/fatcat/release/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata")
    assert res.status_code == 200
    assert r.pages in res.text
    assert r.volume in res.text
...
def test_generic_entity_revision_view_container_view(client, fcclient, es, entities, es_resps):
    ...
    res = client.get("/fatcat/container/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d")
    assert res.status_code == 200
    assert "urusei yatsura" in res.text
...
def test_generic_entity_revision_view_container_view_metadata(client, fcclient, mocker, entities):
    c = entities["container"]
    fcclient.get_container_revision.return_value = c
    res = client.get("/fatcat/container/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata")
    assert res.status_code == 200
    assert "urusei yatsura" in res.text
...
def test_generic_entity_revision_view_creator(client, fcclient, entities):
    ...
    res = client.get("/fatcat/creator/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d")
...
def test_generic_entity_revision_view_creator_metadata(client, fcclient, entities):
    ...
    res = client.get("/fatcat/creator/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata")
...
def test_generic_entity_revision_view_file(client, fcclient, entities):
    ...
    res = client.get("/fatcat/file/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d")
    assert res.status_code == 200
    assert f.md5 in res.text
    assert f.sha256 in res.text
...
def test_generic_entity_revision_view_file_metadata(client, fcclient, entities):
    ...
    res = client.get("/fatcat/file/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata")
    assert res.status_code == 200
    assert f.md5 in res.text
    assert f.sha256 in res.text
    assert "release_ids" in res.text
...
def test_generic_entity_revision_view_fileset(client, fcclient, entities):
    ...
    res = client.get("/fatcat/fileset/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d")
    assert "File Manifest" in res.text
    assert res.status_code == 200
    assert fs.manifest[0].path in res.text
...
def test_generic_entity_revision_view_fileset_metadata(client, fcclient, entities):
    fs = entities["fileset"]
    fcclient.get_fileset_revision.return_value = fs
    res = client.get("/fatcat/fileset/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata")
    assert res.status_code == 200
    assert fs.manifest[0].path in res.text
...
def test_generic_entity_revision_view_webcapture(client, fcclient, entities):
    ...
    res = client.get("/fatcat/webcapture/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d")
    assert res.status_code == 200
    assert wc.cdx[0].sha1 in res.text
...
def test_generic_entity_revision_view_webcapture_metadata(client, fcclient, entities):
    wc = entities["webcapture"]
    fcclient.get_webcapture_revision.return_value = wc
    res = client.get("/fatcat/webcapture/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata")
    assert res.status_code == 200
    assert wc.cdx[0].sha1 in res.text
...
def test_generic_entity_revision_view_work(client, fcclient, entities):
    ...
    #fcclient.get_work_releases.return_value = [entities["release"]]
    res = client.get("/fatcat/work/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d")
    assert res.status_code == 200
    assert w.ident in res.text
...
def test_generic_entity_revision_view_work_metadata(client, fcclient, entities):
    ...
    fcclient.get_work_revision.return_value = w
    res = client.get("/fatcat/work/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/metadata")
    assert res.status_code == 200
    assert w.ident in res.text
    assert "Entity Metadata" in res.text
...
def test_generic_entity_revision_view_release_contribs(client, fcclient, entities):
    ...
    fcclient.get_release_revision.return_value = r
    res = client.get("/fatcat/release/rev/a078e5fe-0815-4ec4-82d8-7841b8a6317d/contribs")
    assert res.status_code == 200
    assert "(the iron man, tetsuo)" in res.text
