...
def container_to_elasticsearch(
    entity: Any, force_bool: bool = True, stats: Dict[str, Any]|None = None
) -> Dict[str, Any]:
    """
...
def file_to_elasticsearch(entity: FileEntity) -> Dict[str, Any]:
    ...
    def join_parsed_url(pu: ExtractResult) -> str:
        parts = []
        for p in [pu.subdomain, pu.domain, pu.suffix]:
            if p:
                parts.append(p)
        return ".".join(parts)

    t["hosts"] = map(join_parsed_url, parsed_urls)
...
def release_to_elasticsearch(entity: ReleaseEntity, force_bool: bool = True) -> Dict[str, Any]:
    ...
    if entity.state in ("redirect", "deleted"):
        return { 'ident': entity.ident, 'state': entity.state }
    elif entity.state != "active":
        raise ValueError("Unhandled entity state: {}".format(entity.state))
    ...
    if extra:
        ...
        if not t["subtitle"] and extra.get("subtitle"):
            if isinstance(extra["subtitle"], list):
...
def _rte_container_helper(container: ContainerEntity, release_year: int|None) -> dict:
    """
...
def check_kbart(year: int, archive: dict) -> bool|None:
...
def test_check_kbart() -> None:
    assert check_kbart(1990, {}) is None
    assert check_kbart(1990, {'year_spans':[[2000, 2000]]}) is False
    assert check_kbart(2000, {'year_spans':[[2000, 2000]]}) is True
    assert check_kbart(1950, {'year_spans':[[1900, 1920], [1990, 2000]]}) is False
    assert check_kbart(1950, {'year_spans':[[1900, 1920], [1930, 2000]]}) is True
