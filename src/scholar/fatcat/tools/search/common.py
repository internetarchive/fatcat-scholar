...
class CatSearchError(Exception):
...
# TODO i don't think we are running 6 anywhere
...
def wrap_es_execution(search: Search) -> Any:
    ...
    try:
        ...
        raise CatSearchError(e.status_code, str(e.error), description)
    except elasticsearch.exceptions.ConnectionError as e:
        raise CatSearchError(e.status_code, "ConnectionError: search engine not available")
